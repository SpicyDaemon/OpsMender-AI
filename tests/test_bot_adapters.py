...
def test_slack_verify_webhook():
    ...
    connector = BotConnector(
        platform="slack", is_enabled=True, credentials={"signing_secret": secret}
    )
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={"x-slack-signature": "wrong"}, raw_body=body
        )
...
def test_discord_verify_webhook():
    ...
    connector = BotConnector(
        platform="discord", is_enabled=True, credentials={"public_key": public_key_hex}
    )
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={"x-signature-ed25519": "wrong"}, raw_body=body
        )
...
def test_discord_parse_inbound():
    ...
    payload = {
        ...
        "channel_id": "C789",
        "member": {"user": {"id": "D123"}},
        "data": {"name": "incident", "options": [{"name": "id", "value": "uuid-abc"}]},
    }
    msg = adapter.parse_inbound(payload)
...
def test_mattermost_verify_webhook():
    ...
    connector = BotConnector(
        platform="mattermost", is_enabled=True, credentials={"webhook_token": token}
    )
...
def test_matrix_verify_webhook():
    ...
    secret = "matrixsecret"
    connector = BotConnector(
        platform="matrix", is_enabled=True, credentials={"webhook_secret": secret}
    )
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={"Authorization": "Bearer wrong"}, raw_body=b"{}"
        )
    assert exc.value.status_code == 403
        ...
        "events": [
            {
                ...
                "content": {"msgtype": "m.text", "body": "/approvals"},
            }
        ]
    }
    msg = adapter.parse_inbound(payload)
...
def test_feishu_verify_webhook():
    ...
    token = "feishutoken"
    connector = BotConnector(
        platform="feishu", is_enabled=True, credentials={"verification_token": token}
    )
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector,
            headers={},
            raw_body=json.dumps({"token": "wrong"}).encode("utf-8"),
        )
    assert exc.value.status_code == 403
...
def test_dingtalk_verify_webhook():
    ...
    secret = "dingsecret"
    connector = BotConnector(
        platform="dingtalk", is_enabled=True, credentials={"app_secret": secret}
    )
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={"timestamp": timestamp, "sign": "wrong"}, raw_body=b"{}"
        )
    assert exc.value.status_code == 403
...
def test_twilio_verify_webhook():
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={"X-Twilio-Signature": "wrong"}, raw_body=body
        )
    assert exc.value.status_code == 403
...
def test_email_verify_webhook():
    ...
    key = "mailgunkey"
    connector = BotConnector(
        platform="email", is_enabled=True, credentials={"mailgun_api_key": key}
    )

    timestamp = "123456"
    token = "mailguntoken"
    hmac_digest = hmac.new(
        key.encode("utf-8"), (timestamp + token).encode("utf-8"), hashlib.sha256
    ).hexdigest()

    payload = {
        "signature": {"timestamp": timestamp, "token": token, "signature": hmac_digest}
    }
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={}, raw_body=json.dumps(payload).encode("utf-8")
        )
    assert exc.value.status_code == 403
...
def test_homeassistant_verify_webhook():
    ...
    with pytest.raises(HTTPException) as exc:
        adapter.verify_webhook(
            connector, headers={"X-Hass-Secret": "wrong"}, raw_body=b"{}"
        )
    assert exc.value.status_code == 403
...
def test_bluebubbles_parse_inbound():
    ...
    payload = {
        "type": "new-message",
        "data": {"text": "/approvals", "handle": {"address": "+1555000"}},
    }
    msg = adapter.parse_inbound(payload)
