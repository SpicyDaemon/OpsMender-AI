...
In AIM, "Skills" are the capabilities provided to the AI agent. AIM uses the **Model Context Protocol (MCP)** to define and execute these skills. By attaching an MCP server to AIM, you grant the AI the ability to interact with your real-world infrastructure.
...
You can manage your skills from the **Skills** tab in the AIM Dashboard.
...
AIM supports two transport mechanisms for MCP servers:

1. **Stdio (Command-line):** The MCP server runs as a local subprocess. AIM communicates with it over standard input/output.
   - *Example:* `python mcp_server.py` or `npx @modelcontextprotocol/server-postgres`
2. **SSE (Server-Sent Events):** The MCP server runs as an independent web service, and AIM connects to it over HTTP.
   - *Example:* `https://my-internal-mcp-server.local/sse`

## 4. Enforcement & Safety Tiers

AIM enforces strict safety guarantees using **Tiers**.
...
When attaching an MCP server to AIM, you map the server to a specific Tier limit. If an AI agent attempts to use a tool that exceeds its current Tier permissions, AIM pauses execution and requests **human approval** in the dashboard.
