...
class TwilioAdapter:
    ...
    def verify_webhook(
        ...
        *,
        ...
        if not auth_token:
            raise HTTPException(
                status_code=403, detail="Twilio auth token not configured"
            )
        ...
        if not webhook_url:
            ...
            raise HTTPException(
                status_code=403,
                detail="Twilio webhook_url must be configured for signature validation",
            )
        ...
        mac = hmac.new(
            auth_token.encode("utf-8"), basestring.encode("utf-8"), hashlib.sha1
        )
    ...
    async def send_message(
        ...
        *,
        ...
        if not all([sid, token, from_number]):
            return (
                False,
                "Twilio credentials (account_sid, auth_token, phone_number) not configured",
            )
