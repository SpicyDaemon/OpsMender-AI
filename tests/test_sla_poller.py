...
class TestSLAPoller:
    ...
    async def test_probe_target_http_success(self, factory, config, db: AsyncSession):
        ...
        target = await SLATargetRepo.create(
            db, name="web", kind="http", config={"url": "http://test.com", "expected_status": 200}
        )
    ...
    async def test_probe_target_http_failure(self, factory, config, db: AsyncSession):
        poller = SLAPoller(factory, config)
        target = await SLATargetRepo.create(
            db, name="web2", kind="http", config={"url": "http://test.com", "expected_status": 200}
        )
    ...
    async def test_probe_target_tcp_success(self, factory, config, db: AsyncSession):
        poller = SLAPoller(factory, config)
        target = await SLATargetRepo.create(
            db, name="db", kind="tcp", config={"host": "127.0.0.1", "port": 5432}
        )
    ...
    @pytest.mark.asyncio
    async def test_probe_and_record_creates_sample(self, factory, config, db: AsyncSession):
        poller = SLAPoller(factory, config)
        target = await SLATargetRepo.create(
            db, name="web3", kind="http", config={"url": "http://test.com"}
        )
    ...
    @pytest.mark.asyncio
    async def test_probe_and_record_suppressed_in_maintenance(self, factory, config, db: AsyncSession):
        poller = SLAPoller(factory, config)
        target = await SLATargetRepo.create(
            db, name="web4", kind="http", config={"url": "http://test.com"}
        )
