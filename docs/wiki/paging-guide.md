...
6. **Configure workspace notification channels** under `/dashboard/paging/notification-channels` for Slack, Teams, Discord, Email, SMS, Telegram, WhatsApp, Signal, or a custom adapter.
7. **Each operator sets their notification preferences** under `/dashboard/paging/my-notifications`. Enable the channel they want paged on, paste the destination (Slack user id, Teams chat id, email, phone), and configure the priority-routing matrix and quiet hours.
8. **Wire the alert source.** Use the service as the home for Alert Intake / Service Webhook setup. v1 still supports the legacy `/dashboard/ingest-tokens` route, but the intended UX is one unique webhook URL per service with an embedded unguessable secret so external monitors can POST without separate API-key headers.
9. **Verify.** POST a synthetic alert with `severity: critical`. The priority rule should fire, the chain should kick off, and the on-call operator should see a Slack/Teams DM (or Email/SMS) within seconds. The end-to-end test `tests/test_e2e_paging_flow.py::TestIncidentResponseLoop` walks the same flow programmatically — useful as a reference for what to assert against.
