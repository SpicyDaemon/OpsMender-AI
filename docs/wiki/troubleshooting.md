...
This guide covers common issues you might encounter while deploying, configuring, or operating the AI Incident Manager (AIM).

## 1. Login Issues

**Symptom:** Unable to log in with the default credentials, or getting a 401 Unauthorized error immediately after deployment.
**Resolution:**
- Verify that the backend database has been properly initialized. AIM uses Alembic for database migrations. Ensure the migration scripts have run successfully.
- If using an external SSO provider or reverse proxy (like an Identity Aware Proxy), ensure the proxy is correctly passing the authenticated user headers (e.g., `X-Forwarded-User`) to the AIM backend, and that AIM is configured to trust that proxy.
...
**Resolution:**
- **For Stdio (Command-line) Servers:** Verify that the command specified in the Skill configuration is available in the AIM backend's system `PATH` (e.g., `python`, `npx`). Ensure any necessary dependencies are installed in that environment.
- **For SSE (Web) Servers:** Ensure the AIM backend container can resolve the hostname of the MCP server. Check for network policies or firewalls blocking traffic between the AIM backend and the SSE endpoint.
- **Logs:** Check the AIM backend application logs for specific traceback errors related to the MCP client subprocess.
...
**Resolution:**
- AIM uses WebSockets for real-time chat updates. Ensure your reverse proxy (e.g., Nginx, Traefik) or Load Balancer is configured to support WebSocket upgrades.
