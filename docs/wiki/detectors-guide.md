...
Detectors in AIM are proactive, LLM-driven rules that continuously monitor your incoming metrics, logs, or signals to identify potential incidents before they cause widespread outages. 
...
Every time a detector runs, AIM logs the result in the detector's **Run History**.
...
LLM-based detection can occasionally trigger false positives. AIM provides tools to fine-tune your detectors:
