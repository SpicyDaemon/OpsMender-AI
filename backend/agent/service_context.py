...
service is, how important it is, and which MCP tools the operators prefer for it.
...
def format_service_context(
    *,
    ...
    preferred_mcp_names: list[str] | None = None,
) -> str:
    ...
    names = [n.strip() for n in (preferred_mcp_names or []) if n and n.strip()]
    if names:
        lines.append("- Preferred MCP servers: " + ", ".join(names))
    return "\n".join(lines)
