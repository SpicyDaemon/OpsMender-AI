...
* [AI Incident Memory](memory-guide.md) — How OpsMender carries lessons across incidents, the `recall` / `remember` workflow nodes, auto-compaction, the operator thumbs-up/down loop, and what is and isn't remembered. `/dashboard/memories` is the curation surface; the session detail page has a "Memories used" panel showing what shaped each session.
