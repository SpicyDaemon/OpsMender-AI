...
class SessionModelSwitchRequest(BaseModel):
...
class SessionResponse(BaseModel):
    ...
    allowed_model_config_ids: list[uuid.UUID] = Field(default_factory=list)
    tier: int
...
class StatusPageSettingsUpdate(BaseModel):
...
class StatusPageComponentInput(BaseModel):
    service_id: uuid.UUID
    display_name: Optional[str] = Field(default=None, max_length=150)


class StatusPageComponentReplaceRequest(BaseModel):
    components: list[StatusPageComponentInput] = Field(default_factory=list)
...
class StatusPageComponentListResponse(BaseModel):
    items: list[StatusPageComponentResponse]
    total: int


class StatusPageUpdateCreate(BaseModel):
    state: str = Field(pattern="^(investigating|identified|monitoring|resolved)$")
    body: str = Field(min_length=1, max_length=5000)


class StatusPageUpdateResponse(BaseModel):
    id: uuid.UUID
    incident_id: uuid.UUID
    state: str
    body: str
...
class StatusPageUpdateListResponse(BaseModel):
    items: list[StatusPageUpdateResponse]
    total: int


class StatusPageSubscriberResponse(BaseModel):
    id: uuid.UUID
    email: str
...
class StatusPageSubscriberListResponse(BaseModel):
    items: list[StatusPageSubscriberResponse]
    total: int
...
class StatusPageUptimeDay(BaseModel):
    date: str
    pct: float
...
class PublicStatusComponent(BaseModel):
    ...
    display_name: str
    status: str
    uptime_90d: Optional[list[StatusPageUptimeDay]] = None


class PublicStatusIncidentUpdate(BaseModel):
    state: str
    body: str
    published_at: datetime
...
class PublicStatusIncident(BaseModel):
    ...
    title: str
    priority: Optional[str] = None
    updates: list[PublicStatusIncidentUpdate]
